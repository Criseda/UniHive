...
// get the first listing image for a listing
router.get("/listing/:listing_id/first", async (req, res) => {
  const { listing_id } = req.params;
  try {
    const listingImage = await pool.query(
      "SELECT * FROM listing_image WHERE listing_id = $1 ORDER BY id ASC LIMIT 1",
      [listing_id]
    );
    res.json(listingImage.rows[0]);
  } catch (err) {
